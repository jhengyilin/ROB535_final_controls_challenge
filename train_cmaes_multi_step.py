...
class CMAESConfig:
    ...
    controller_name: str = "custom_multi_step"
...
def train_cmaes(config: CMAESConfig):
    ...
    def callback(gen, best_cost, best_params):
        ...
        save_params(best_params_phys, "optimized_params_multistep.json", gen, 
                   best_cost, config.population_size, hyperparameters=hyperparameters)
        
        if gen % 5 == 0 and gen > 0:
            save_params(best_params_phys, f"checkpoint_multistep_gen{gen}.json", gen, 
                       best_cost, config.population_size, hyperparameters=hyperparameters)
    ...
    save_params(best_params, "optimized_params_multistep_final.json", optimizer.generation, 
...
if __name__ == "__main__":
    ...
    if best_params is not None:
        ...
        for k, v in param_dict.items():
            print(f"  '{k}': {v:.6f},")
        print(f"\n✅ Training complete! Parameters saved to optimized_params_multistep_final.json")